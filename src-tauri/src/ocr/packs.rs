...
fn packs_root(app: &AppHandle) -> Result<PathBuf> {
    let dir = app
        .path()
        .app_local_data_dir()
        .map_err(|err| AppError::Other(anyhow::anyhow!("resolve app_local_data_dir: {err}")))?
        .join("ocr-packs");
    Ok(dir)
}
...
pub fn list(app: &AppHandle) -> Vec<PackStatus> {
    PACKS
        ...
        .map(|pack| PackStatus {
            ...
            total_bytes: pack.rec_bytes + pack.dict_bytes,
        })
        .collect()
}
...
pub fn delete(app: &AppHandle, id: &str) -> Result<()> {
    ...
    if dir.is_dir() {
        std::fs::remove_dir_all(&dir)
            .map_err(|err| AppError::Other(anyhow::anyhow!("delete ocr pack {id}: {err}")))?;
    }
...
async fn download_inner(
    ...
    for (file, expected) in [
        ...
        fetch_file(app, pack.id, file, &target, expected, &mut received, total).await?;
    }
    super::invalidate_engine();
    let _ = app.emit(
        PACK_PROGRESS_EVENT,
...
async fn fetch_file(
    ...
    for url in &urls {
        match fetch_one(app, pack_id, url, target, expected, *received, total).await {
            Ok(()) => {
                *received += expected;
                return Ok(());
            }
...
async fn fetch_one(
    ...
    let mut writer = std::io::BufWriter::new(
        std::fs::File::create(&part)
            .map_err(|err| AppError::Other(anyhow::anyhow!("create {part:?}: {err}")))?,
    );
    ...
    while let Some(chunk) = stream.next().await {
        ...
        std::io::Write::write_all(&mut writer, &chunk)
            .map_err(|err| AppError::Other(anyhow::anyhow!("write {part:?}: {err}")))?;
        ...
        if last_emit.elapsed().as_millis() >= 150 {
            ...
            let _ = app.emit(
                ...
                PackProgress {
                    id: pack_id.to_owned(),
                    received: base_received + written,
                    total,
                    error: None,
                },
            );
    ...
    std::io::Write::flush(&mut writer)
        .map_err(|err| AppError::Other(anyhow::anyhow!("flush {part:?}: {err}")))?;
    drop(writer);
    ...
    std::fs::rename(&part, target)
        .map_err(|err| AppError::Other(anyhow::anyhow!("rename {part:?}: {err}")))?;
    Ok(())
}
