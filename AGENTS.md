...
- **提交与推送分支策略**：需要推送代码且当前分支是 `master` 时，自动新建工作分支，在新分支提交并推送；当前分支不是 `master` 时，先询问用户是在当前分支提交并推送，还是新建分支后提交并推送。**例外**：发版流程（版本号 bump、changelog、tag，见「发版流程」一节）及用户明确指示的修复推送可以直推 `master`。
